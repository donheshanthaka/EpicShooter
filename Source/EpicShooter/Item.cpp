...
void AItem::BeginPlay()
{
	...
	// Sets ActiveStars array based on ItemRarity
	SetActiveStars();
...
void AItem::ItemInterp(float DeltaTime)
{
	...
	if (Character && ItemZCurve) {
		...
		const float ElapsedTime = GetWorldTimerManager().GetTimerElapsed(IntemInterpTimer);
		...
		// Adding curve value to the Z component of the initial location (saled by DeltaZ)
		...
		if (ItemScaleCruve) {
			const float ScaleCurveValue = ItemScaleCruve->GetFloatValue(ElapsedTime);
...
void AItem::UpdatePulse()
{
	...
	case EItemState::EIS_EquipInterping:
		if (InterpPulseCurve) {
			ElapsedTime = GetWorldTimerManager().GetTimerElapsed(IntemInterpTimer);
