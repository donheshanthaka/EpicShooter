...
AShooterCharacter::AShooterCharacter() :
	...
	AutomaticFireRate(0.1f), // If you increase this value, you should also increase CrosshairSpreadMultiplier (default 0.5f)
	bShouldFire(true),
...
void AShooterCharacter::CalculateCrosshairSpread(float DeltaTime)
{
	...
	if (GetCharacterMovement()->IsFalling()) { // if in air
		...
		CrosshairInAirFactor = FMath::FInterpTo(CrosshairInAirFactor, 2.0f, DeltaTime, 2.25f);
	}
	...
	if (bAiming) { // if aiming
		// Shrink crosshairs by a small amount
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType)) {
		...
		// Amount of ammo the character is carrying of the the EquippedWeapon type
		...
		if (MagEmptySpace > CarriedAmmo) {
			// Reload the magazine withh all the ammo we are carrying
